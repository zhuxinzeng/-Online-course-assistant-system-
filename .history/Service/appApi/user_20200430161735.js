...
router.post('/passwordinfo', async(ctx)=>{
    ...
    const User = mongoose.model('User');
    await User.updateOne({_id:id},{$set:{'password':password}}).then(() => {
        ctx.body = {
            code: 200,
            message: '修改密码成功'
        }
...
router.post('/contactinfo', async(ctx)=>{
    ...
    const User = mongoose.model('User');
    await User.updateOne({_id:id},{$set:{'email':email,'tel':tel},}).then(() => {
        ctx.body = {
            code: 200,
            message: '修改成功'
        }
...
router.post('/getalluserinfo', async (ctx) => {
    try{
        ...
        let result =await adminuser.find().skip((pageNumber-1)*8).limit(8).exec()
