...
router.post('/useridinsert', async (ctx) => {
    ...
    const Courses = mongoose.model('Courses');
    await Courses.update({COURSE_NAME:coursename},{$push:{"USERID":userid}}).then(()=>{
       ctx.body={
           code:200,
           message:'成功加入我的课程'
       }
...
router.post('/getmycourses',async (ctx)=>{
    ...
    const Courses = mongoose.model('Courses');
    await Courses.find({USERID:userid}).exec().then((result)=>{
        ctx.body = {
            code:200,
            message:result
        }
...
router.post('/deletemycourses',async (ctx)=>{
    ...
    const Courses = mongoose.model('Courses');
    await Courses.updateMany({COURSE_NAME:coursename},{$pull:{'USERID':userid}}).exec().then((result)=>{
        ctx.body = {
            code:200,
            message:result 
        }
