...
router.post('/getexaminfo', async (ctx) => {
   ...
   // .skip((page - 1) * 8).limit(8).
    const Exam = mongoose.model('Exam');
    await Exam.find({examtime:120}).exec().then(result => {
        ctx.body = {
            code: 200,
            message: result
...
router.post('/getexamoptionsinfo', async (ctx) => {
   // .skip((page - 1) * 8).limit(8).
    const Exam = mongoose.model('Exam');
    await Exam.find({examname:"Java"}).exec().then(result => {
        let ques_sequence = result[0].ques_sequence;
        console.log(ques_sequence)
        ctx.body = {
            code: 200,
            message: ques_sequence
