<template>
  <div class="content">
    ...
    <a-row>
      <a-col :span="20">
        <div class="grid-content bg-purple">
          <a-input v-model="inValue" size="small" placeholder="请输入内容" @keyup.enter.native="btn_sbmit"></a-input>
        </div>
      </a-col>
    ...
    <ul>
      ...
      <li v-for="(item,index) in userIp" :key="index" :class="{active:item.name == userStorage}" >
        <span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        inValue: '', //当前输入信息
        userIp: '',
        userStorage: localStorage.getItem("userInfo"),
    ...
    mounted() {
        this.userIp=this.$store.state.user;
    ...
    methods: {
      ...
      btn_sbmit() {
        ...
        //   this.userIp = this.formatDateTime() + Math.random().toString(36).substr(2);
          ...
          this.$socket.emit("send", { name: '' + this.userIp+':', getMsg: this.inValue })
