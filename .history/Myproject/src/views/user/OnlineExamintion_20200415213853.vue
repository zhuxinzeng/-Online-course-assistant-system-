<template>
  <div class="OE-container">
    <Nav />
    <div class="top" :class="{showtopbox:showtopbox}" >
    ...
    <div class="examinfo" :class="{examInfo:showbox}">
      <div class="detail">
        ...
        <span>
           <b> 考试名称: </b>
          <p>{{examinfo[0].subjectname}}</p>
        </span>
        
        
        <span> 
         <b> 考试时间: </b>
          <p>{{examinfo[0].examtime}}</p>
        </span>

        <span>
        <b>  考试分值:</b>
          <p>{{examinfo[0].examscore}}</p>
        </span>
        <span>
          <b>考生姓名:</b>
          <p>{{username}}</p>
      ...
      <div class="startbtn"  @click="hideallbox()">
        <a-button>开始考试</a-button>
      </div>
      <div class="examinfo" v-for="(item,index) in examinfo"  :key="index">
           {{examinfo[index].ques_sequence}}
              {{examinfo[index].ques_title}}
            <div v-for="(item,index) in examinfo[index].options_list[index] " :key="index"> {{examinfo[index].options_list[index].options_title}}</div>
