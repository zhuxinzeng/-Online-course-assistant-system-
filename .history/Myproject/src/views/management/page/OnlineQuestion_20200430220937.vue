...
export default {
  ...
  data() {
    return {
      ...
      inValue: "", //当前输入信息
      userName: "",
      userStorage: sessionStorage.getItem("adminusername"),
  ...
  mounted() {
    if (this.$store.state.userName) {
      this.userName = this.$store.state.adminusername;
    } else {
      this.userName = JSON.parse(localStorage.getItem("adminusername"));
    }
...
.btninput {
  ...
  box-shadow: 1px 5px 10px #bdc3c7;
