<template>
  <div class="EXD-container">
    ...
    <div class="EXD-content" v-for="(item,index) in examinfo" :key="index">
     ...
     {{item.ques_title}}  
      ...
      <a-checkbox-group @change="onChange">
        <a-row>
          <a-col :span="12">
            <a-checkbox value="A" v-model="checkedValue">A.{{item.options_list[0].options_title}}</a-checkbox>
          </a-col>
          <a-col :span="12">
    ...
    <div class="resultlist">第一题：{{this.answer}}</div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    onChange(checkedValues) {
      ...
      this.answer=[];
      this.answer.push(this.checkedValues);
