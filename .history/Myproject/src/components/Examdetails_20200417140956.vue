        ...
        <div class="content-option">
          <a-row>
            <a-col :span="12" class="optionspace" >
              <a-checkbox value="A" @change="onChange" :checked="checked">A.{{item.options_list[0].options_title}}</a-checkbox>
            </a-col>
            <a-col :span="12" class="optionspace">
              <a-checkbox value="B" @change="onChange" :checked="checked">B.{{item.options_list[1].options_title}}</a-checkbox>
            </a-col>
            <a-col :span="12" class="optionspace">
              <a-checkbox value="C" @change="onChange" :checked="checked">C.{{item.options_list[2].options_title}}</a-checkbox>
            </a-col>
            <a-col :span="12" class="optionspace">
              <a-checkbox value="D" @change="onChange" :checked="checked">D.{{item.options_list[3].options_title}}</a-checkbox>
            </a-col>

            <a-button class="EXD-btn" @click="next">
              <p @click="getdata()">下一题</p>
            </a-button>
             <a-button class="EXD-btn" >
              <p @click="againchoose()">重新选择</p>
            </a-button>本题选择:{{answer}}
          </a-row>
        </div>
        <!-- </div> -->
      </div>
...
export default {
  ...
  onChange(checkedValues) {
      ...
      this.answer.push(checkedValues.target.value)
      this.checked=true;
      console.log(`这个checked = ${checkedValues.target.checked}`);
