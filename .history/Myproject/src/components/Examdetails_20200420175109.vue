<template>
  <div class="EXD-container">
      {{examSingleinfo[0].Topictypes}}
    <div class="time">
      <a-affix :offsetTop="this.top">
      <span> 距离考试结束还有</span> <p id="timer"></p>
      </a-affix>
    </div>
    <div class="middle-content">
      <div v-if="examSingleinfo.Topictypes=='单选题'"  >
      <div v-if="examJudge.Topictypes=='判断题'"  >
         <div class="EXD-content"  v-for="(item,index) in examJudge" :key="index">
        ({{item.Topictypes}})
        {{item.ques_sequence}}
        {{item.ques_title}}
        <!-- {{item.options_list}} -->
  ...
  methods: {
    getdata(number) {
      ...
      axios({
        ...
        console.log(response.data.message[0].Topictypes)
        if (response.status == 200 && response.data.message[0].Topictypes =='单选题') {
        ...
        } else if(response.status == 200 && response.data.message[0].Topictypes =='判断题'){
