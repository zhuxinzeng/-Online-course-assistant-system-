        ...
        <div class="content-option">
          <a-row>
            ...
            <a-button class="EXD-btn" @click="next">
              <p @click="getdata()"><span @click="getresult()">下一题</span> </p>
            </a-button>
            <span @click="getresult()">test</span> 
  ...
  methods: {
    ...
    getresult(){
      ...
      axios({
        ...
        console.log(response);
        if (response.status == 200) {
           this._id=response.data.message[0]._id;
        this.isMatch=response.data.message
         this.$message.success('后台传回_id'+this._id)
         this.$message.success('后台传回_'+this.isMatch)
        //   this.$store.commit("saveExaminfo", this.examinfo);
        } else {
          this.$message.error("服务器端错误！");
        }
